...
class TestQueryEverything:
    ...
    def test_fatal_field_error_raises(self):
...
class TestUpdatePullRequests:
    ...
    def test_transient_status_drops_a_comment_github_already_added(self):
        """A comment already on the pr landed before the 502, so it isn't resent."""
        ep = make_endpoint()
    ...
    def test_transient_status_resends_only_the_missing_comment(self):
        """A lost comment is resent while one that landed in the same request is dropped."""
        ep = make_endpoint()
    ...
    def test_repeated_transient_status_stops_resending(self):
        """A mutation that keeps failing gives up instead of resending forever."""
        ep = make_endpoint()
    ...
    def test_transient_status_with_only_added_comments_sends_nothing_more(self):
        """A mutation whose every comment already landed sends nothing more."""
        ep = make_endpoint()
        ep._post = commenting_post({"PR_1": ["hi"]})
        gh = make_github(ep)
    ...
    def test_fatal_mutation_error_raises(self):
        endpoint = AsyncMock(spec=GitHubEndpoint)
        endpoint.graphql = AsyncMock(
            return_value=gql(
                {"pr_out0": None},
    ...
    def test_repeated_timeout_gives_up_after_stall_limit(self):
        ...
        of times, then surfaced as a fatal error rather than looping forever."""
        endpoint = AsyncMock(spec=GitHubEndpoint)
        call_count = 0
...
def commenting_post(comments_by_pr, fail_forever=False):
    """An async _post replacement that 502s a mutation adding comments, answers the
    resulting lookup from `comments_by_pr`, and succeeds otherwise.
