...
def make_passthrough_editor_script(tmp_dir):
    path = Path(tmp_dir) / "editor.sh"
    tmp_file = Path(tmp_dir) / "_revup_edit_tmp"
    path.write_text(f'#!/bin/sh\nhead -1 "$1" > {tmp_file}\nmv {tmp_file} "$1"\n')
    path.chmod(stat.S_IRWXU)
    return str(path)
...
class GitTestEnvironment:
    def __init__(self):
        self.tmp_dir = Path(tempfile.mkdtemp(prefix="revup_test_"))
        self.sh = shell.Shell(quiet=True, cwd=str(self.tmp_dir))
        self.git_ctx = None
        self.commit_count = 0
    ...
    async def setup(self):
        ...
        await self.sh.sh(self._git_path, "config", "user.name", TEST_AUTHOR_NAME)
        await self.sh.sh(self._git_path, "config", "user.email", TEST_AUTHOR_EMAIL)
        ...
        self.git_ctx.repo_root = str(self.tmp_dir)
