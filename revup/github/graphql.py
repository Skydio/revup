...
class GraphqlOperation(str, Enum):
    """The top-level GraphQL operation type. str-valued so it renders as itself."""
...
class ErrorClass(Enum):
    ...
    # An informational notice that accompanies an otherwise-successful response.
    INFORMATIONAL = "informational"
    # Won't succeed on retry (not found, no permission, malformed).
    FATAL = "fatal"
...
class GraphqlError:
    ...
    def error_class(self) -> ErrorClass:
        return _ERROR_CLASS_BY_TYPE.get(self.type, ErrorClass.FATAL)
...
class GraphqlQuery:
    ...
    def build(self) -> Tuple[str, Dict[str, Any]]:
        ...
        if self.operation == GraphqlOperation.QUERY:
            body = ""
            if repo_fields:
                body += f"""
            ...
            body += top_fields
            query_str = f"""
        {self.operation.value}{name_str} ({decl_str}) {{{body}
        }}"""
        else:
            query_str = f"""
        {self.operation.value}{name_str} ({decl_str}) {{
            {mutation_fields}
        }}"""
