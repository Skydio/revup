...
# Tags that don't determine which pr a commit belongs to, so a cherry-picked commit still
# reaches the same pr without them.
NONIDENTIFYING_TAGS = {
    TAG_LABEL,
    TAG_REVIEWER,
    TAG_ASSIGNEE,
...
class TopicStack:
    ...
    async def populate_relative_reviews(
        self, uploader: str, branch_format: str, deep_stack_draft: int
    ) -> None:
        for name, topic in self.topological_topics():
            if topic.relative_topic:
